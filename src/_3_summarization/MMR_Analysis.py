...
def MMR_Analysis(query, top_ss, topnum, w2v_model, idf_vocab, stopword):
    # print 'MMR analysis', time.strftime('%Y-%m-%d %H:%M:%S')
    sim_matrix = build_sim_matrix(query, top_ss, w2v_model, idf_vocab, stopword)
    rank_list = MMR_Algorithm(sim_matrix, topnum)
    selected_sentence = []
    #print rank_list
    for rank in rank_list:
        selected_sentence.append(top_ss[rank].capitalize())
...
def build_sim_matrix(query, top_ss, w2v_model1, idf_vocab1, stopword1):
    # add query
...
def MMR_Algorithm(sim_matrix, topnum):
    ...
    # init
    Set = []
    Rest = [i for i in range(0, query_index, 1)]
    #print Rest
    # parameter
    Lambda = 0.5
    while iteration <= topnum:
        ...
        for i in range(0, query_index, 1):
            if sim_matrix[i][query_index] > max_dq_sim:
                ...
                #print most_sim_with_query_index
        if len(Set) == 0:
